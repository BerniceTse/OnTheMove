...
class SearchViewController: UIViewController
{
    ...
    func getItems()
    {
        ...
        //locate data on Firebase by hierarchy
        ref.child(Constants.Storyboard.FirebaseUser).child(uid!).child(Constants.Storyboard.FirebaseItems).observeSingleEvent(of: .value)
            { (DataSnapshot) in
               if let dict = DataSnapshot.value as? [String: AnyObject]
               {
                  for itemName in dict.keys
                  {
                    //create itemList when looping through Firebase
                    ...
                    ref.child(Constants.Storyboard.FirebaseUser).child(uid!).child(Constants.Storyboard.FirebaseItems).child(itemName).observe(.value)
                        ...
                        //create item model with Firebase data
                        if let dict =  DataSnapshot.value as? [String: AnyObject]
                        {
                            self.itemToStore = dict[Constants.Storyboard.ItemName] as? String
                            self.roomToStore = dict[Constants.Storyboard.ItemRoom] as? String
                            self.boxToStore = dict[Constants.Storyboard.ItemBox] as? String
                            self.quantityToStore = dict[Constants.Storyboard.ItemQuantity] as? String
                            self.descriptionToStore = dict[Constants.Storyboard.ItemDescription] as? String
...
extension SearchViewController: UITableViewDelegate
{
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        for item in itemModelList
        {
            ...
            {
            //pass item information to item view controller
